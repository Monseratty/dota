...
export interface S3ReplayStorageConfig {
  endpoint: string;
  region: string;
  bucket: string;
  accessKeyId: string;
...
export type ReplayStorageConfig =
  ...
  | { driver: "s3"; s3: S3ReplayStorageConfig };
...
function normalizeReplayStorage(
  ...
  const missing = Object.entries(config)
    .filter(([key, value]) => ["endpoint", "region", "bucket", "accessKeyId", "secretAccessKey"].includes(key) && !value)
    .map(([key]) => key);

  if (missing.length > 0) {
    throw new Error(`S3 replay storage is enabled but missing: ${missing.join(", ")}`);
  }

  return { driver: "s3", s3: config };
}
